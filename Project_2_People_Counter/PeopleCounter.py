...
mask=cv2.imread("Project_2_People_Counter/mask.png")
#Tracking 
#max_age=limit of number of frame we can still recognize the car when it come back 

tracker=Sort(max_age=20,min_hits=3,iou_threshold=0.3)

line_limit_Up=[103,161,296,161] 
line_limit_Down=[527,489,735,489] 
...
while True:
    ...
    imgGraphics=cv2.imread("Project_2_People_Counter/graphics.png",cv2.IMREAD_UNCHANGED)
    ...
    cv2.line(frame,(line_limit_Up[0],line_limit_Up[1]),(line_limit_Up[2],line_limit_Up[3]),(0,0,255),5)
    cv2.line(frame,(line_limit_Down[0],line_limit_Down[1]),(line_limit_Down[2],line_limit_Down[3]),(0,0,255),5)
    ...
    for result in resultsTracker:
        ...
        if line_limit_Up[0] < cx < line_limit_Up[2] and line_limit_Up[1]-15 < cy < line_limit_Up[1]+15:
            ...
            if totalCount_Up.count(id)==0:
                ...
                cv2.line(frame,(line_limit_Up[0],line_limit_Up[1]),(line_limit_Up[2],line_limit_Up[3]),(0,255,0),5)
        if line_limit_Down[0] < cx < line_limit_Down[2] and line_limit_Down[1]-15 < cy < line_limit_Down[1]+15:
            #only append unique Id otherwise count increment many
            #times at the particular regions
            if totalCount_Down.count(id)==0:
                totalCount_Down.append(id)
                cv2.line(frame,(line_limit_Down[0],line_limit_Down[1]),(line_limit_Down[2],line_limit_Down[3]),(0,255,0),5)


    # # cvzone.putTextRect(frame,f' Count: {len(totalCount)}',(50,50))

    # #for graphics
    cv2.putText(frame,str(len(totalCount_Up)),(929,345),cv2.FONT_HERSHEY_PLAIN,5,(139,195,75),8)
    cv2.putText(frame,str(len(totalCount_Down)),(1191,345),cv2.FONT_HERSHEY_PLAIN,5,(50,50,250),8)
